...
CREATE TABLE `order_info` (
  ...
  `order_release_time` datetime NOT NULL COMMENT '订单释放时间',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_vin_order` (`vin`,`order_release_time`),
  KEY `idx_brand` (`brand_id`),
  KEY `idx_vin` (`vin`),
...
CREATE TABLE `route_otd_config` (
  ...
  `not_departed_otd` int DEFAULT NULL COMMENT '未出库 → 集港在途 标准时效',
  `to_port_otd` int DEFAULT NULL COMMENT '集港在途 → 已集港待装船 标准时效',
  `at_port_wait_otd` int DEFAULT NULL COMMENT '已集港待装船 → 水运在途 标准时效',
  `on_sea_otd` int DEFAULT NULL COMMENT '水运在途 → 已到港待卸船 标准时效',
  `at_dest_wait_otd` int DEFAULT NULL COMMENT '已到港待卸船 → 已卸船待分拨 标准时效',
  `unload_wait_dispatch_otd` int DEFAULT NULL COMMENT '已卸船待分拨 → 分拨在途 标准时效',
  `dispatching_otd` int DEFAULT NULL COMMENT '分拨在途 → 已到达 标准时效',
  `not_departed_warn` int DEFAULT NULL COMMENT '未出库预警时效',
  `to_port_warn` int DEFAULT NULL COMMENT '集港在途预警时效',
  `at_port_wait_warn` int DEFAULT NULL COMMENT '已集港待装船预警时效',
  `on_sea_warn` int DEFAULT NULL COMMENT '水运在途预警时效',
  `at_dest_wait_warn` int DEFAULT NULL COMMENT '已到港待卸船预警时效',
  `unload_wait_dispatch_warn` int DEFAULT NULL COMMENT '已卸船待分拨预警时效',
  `dispatching_warn` int DEFAULT NULL COMMENT '分拨在途预警时效',
  `is_active` tinyint DEFAULT '1' COMMENT '是否启用',
  PRIMARY KEY (`id`),

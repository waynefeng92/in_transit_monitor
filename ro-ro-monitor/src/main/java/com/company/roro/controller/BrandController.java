...
public class BrandController {
     ...
     * @return 品牌列表
     */
    @ApiOperation(value = "查询所有品牌", notes = "返回品牌列表，可通过参数控制是否包含已禁用")
    @GetMapping("/list")
     ...
     * @return 品牌信息
     */
    @ApiOperation(value = "根据ID查询品牌", notes = "返回指定ID的品牌详细信息")
    @GetMapping("/{id}")
     ...
     * @return 是否成功
     */
    @ApiOperation(value = "新增品牌", notes = "添加新的品牌信息，品牌名称不能重复")
    @PostMapping
    public boolean save(
            @ApiParam(value = "品牌信息", required = true)
    ...
    public boolean update(
            @ApiParam(value = "品牌信息（必须包含ID）", required = true)
            @RequestBody BrandDict brandDict) {
     ...
     * @return 是否成功
     */
    @ApiOperation(value = "删除品牌", notes = "软删除，将 is_active 设置为 0")
    @DeleteMapping("/{id}")
